...
This is a slightly altered version of `blescan.py` from the Bluepy repository.
...
class ScanPrint(btle.DefaultDelegate):
    ...
    def __init__(self, opts):
        ...
        self.bdd = BluetoothDeviceDatabase()
    ...
    def handleDiscovery(self, dev, isNewDev, isNewData):
        ...
        for (sdid, desc, val) in dev.getScanData():
            if sdid in [8, 9]:
                self.bdd.add_device(dev.addr, val, dev.rssi)
                print ('\t' + desc + ': \'' + ANSI_CYAN + val + ANSI_OFF + '\'')
            else:
                self.bdd.add_device(dev.addr, None, dev.rssi)
                print ('\t' + desc + ': <' + val + '>')
        if not dev.scanData:
            print ('\t(no data)')
        print
